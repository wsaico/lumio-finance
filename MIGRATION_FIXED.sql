-- ============================================
-- LUMIO FINANCE - MIGRATION SQL (FIXED)
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'category_learning'
        AND policyname = 'Users can view their own category learning'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'category_learning'
        AND policyname = 'Users can insert their own category learning'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'category_learning'
        AND policyname = 'Users can update their own category learning'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'category_learning'
        AND policyname = 'Users can delete their own category learning'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Anyone can view currencies'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'exchange_rates'
        AND policyname = 'Authenticated users can insert exchange rates'
    ) THEN
...
-- 5. FUNCIÓN para limpiar category_learning antiguos (opcional - ejecutar periódicamente)
...
-- 6. FUNCIÓN trigger para actualizar updated_at automáticamente
...
-- 2. Las tablas budgets, transactions y profiles YA tienen los campos necesarios
-- 3. Este SQL solo crea las 3 nuevas tablas: category_learning, currencies, exchange_rates
-- 4. Las políticas RLS garantizan seguridad a nivel de fila
-- 5. Los índices mejoran el performance de las consultas
